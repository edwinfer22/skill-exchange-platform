...
# Usa solo letras y números en usuario y contraseña, sin acentos ni caracteres especiales
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://skill_user:10092007@localhost/skill_exchange_db"
)
# ...existing code...
engine = create_engine(DATABASE_URL)  # <-- Elimina encoding="utf-8"
# ...existing code...
...
async def search_users(skill: str, db: Session = Depends(get_db)):
    users = db.query(DBUser).filter(DBUser.skills_offered.ilike(f"%{skill}%")).all()
    return users
# ...existing code...